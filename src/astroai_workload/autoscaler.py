...
Implements :class:`ray.autoscaler._private.node_provider.NodeProvider` so Ray's
own autoscaler (started on the head with ``--autoscaling-config``) can launch
and terminate Skaha ``ray-worker`` sessions on demand. Node IDs are CANFAR
session IDs.
...
try:  # pragma: no cover - exercised in images where ray is installed
    from ray.autoscaler._private.node_provider import NodeProvider as _RayNodeProvider
except Exception:  # noqa: BLE001 — keep the module importable without ray (py3.13 images)
